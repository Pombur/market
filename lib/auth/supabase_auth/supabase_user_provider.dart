...
class VodomerkaSupabaseUser extends BaseAuthUser {
  VodomerkaSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> vodomerkaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = VodomerkaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
